...
const refreshAllPrices = async () => {
  ...
  await Promise.allSettled(
    assets.value.map(async (asset) => {
      try {
        const res = await fetch(`http://127.0.0.1:8000/api/quote/${asset.symbol}`)
...
const openHistoryModal = async (asset: Asset) => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:8000/api/history/${asset.symbol}`)
...
const fetchLiveQuote = async () => {
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:8000/api/quote/${symbol}`);
    if (!response.ok) {
